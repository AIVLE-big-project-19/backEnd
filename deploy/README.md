...
이 브랜치(`infra/ecs-deploy`)에서 수동으로 생성한 AWS 리소스 기록. 계획 문서는 `docs/superpowers/plans/`에 있고(예: `2026-08-10-policy-agent-deploy.md`), 이 README는 실제 생성된 리소스 ID를 남기는 기록임.
...
- **보안그룹**: `solaraivle-alb-sg`(80 from 0.0.0.0/0), `solaraivle-ecs-task-sg`(8080 from alb-sg), `solaraivle-rds-sg`(3306 from task-sg), `solaraivle-redis-sg`(6379 from task-sg), `solaraivle-vision-ai-sg`, `solaraivle-ranking-ml-sg`, `solaraivle-chatbot-sg`, `solaraivle-policy-agent-sg`
...
- **AI 서버 연동 상태** — `vision-ai`, `ranking-ml`, `chatbot`, `policy-agent` 4개 서비스는 모두 `solaraivle-cluster`에 내부 전용 ECS 서비스로 배포됨. 이 중 `vision-ai`, `ranking-ml`, `policy-agent` 3개는 Cloud Map(`solaraivle.internal`)으로 연결되고, `chatbot`은 Cloud Map 서비스 등록 없이 ALB 타깃그룹으로 연결됨. `AI_SERVER_URL`만 여전히 `localhost` placeholder로 남아 있음(별도 작업 필요).
