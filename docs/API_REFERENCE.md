...
- 성공: 200 `"회원 탈퇴가 완료되었습니다."` — **모든 개인정보(계정·동의이력·로그인토큰)가 즉시 완전 삭제**되고 복구할 수 없습니다. 작성한 게시글/댓글은 삭제되지 않고 작성자가 "탈퇴한 사용자"로 표시됩니다.
  - 단, 게시글 익명화는 best-effort입니다 — 게시글 작성 시 입력된 `writer` 값이 탈퇴한 계정의 로그인 아이디와 정확히 일치하는 경우에만 "탈퇴한 사용자"로 바뀝니다. `writer`는 현재 게시글 작성 시점에 자유 입력되는 값이라, 실제 작성자와 다른 값이 들어간 게시글은 탈퇴 후에도 원래 writer 값이 그대로 남을 수 있습니다.
