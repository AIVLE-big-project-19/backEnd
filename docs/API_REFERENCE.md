...
- 성공: 200 `"회원 탈퇴가 완료되었습니다."` — **모든 개인정보(계정·동의이력·로그인토큰)가 즉시 완전 삭제**되고 복구할 수 없습니다. 작성한 게시글/댓글은 삭제되지 않고 작성자가 "탈퇴한 사용자"로 표시됩니다.
  - 게시판 모듈이 작성자를 실제 연관관계(author)로 기록한 게시글(최근 게시판 개편 이후 작성분)은 정확히 익명화됩니다. 그 이전에 작성돼 작성자 연관관계가 없는 구버전 게시글은 여전히 best-effort입니다 — 작성 당시 입력된 `writer` 값이 탈퇴한 계정의 로그인 아이디와 정확히 일치하는 경우에만 "탈퇴한 사용자"로 바뀝니다.
