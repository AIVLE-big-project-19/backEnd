...
/**
 * timeoutBoundRestClientBuilder()가 만드는 클라이언트는 cleartext HTTP/2(h2c) 업그레이드를
 * 시도하지 않아야 한다 -- AI 서버(uvicorn/h11)가 Upgrade 헤더가 실린 요청을 프로토콜 단계에서
 * 거부해 모든 호출이 실패하는 문제가 실제로 있었다.
 */
class RestClientConfigTest {
    ...
    void HTTP2_업그레이드를_시도하지_않는다() throws Exception {
        ...
        RestClient.Builder builder = config.timeoutBoundRestClientBuilder(5_000L);
